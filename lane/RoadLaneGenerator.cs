...
public class RoadLaneGenerator : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        GameObject entranceCheckpoint = CreateCheckpoint("entrance",
            transform.position - (transform.forward * (laneLength / 2f)), 
        ...
        for (int i = 1; i <= 3; i++)
        {
            ...
            GameObject rewardCheckpoint = CreateCheckpoint($"reward_{i}",
                transform.position + (transform.forward * distanceFromStart),
        ...
        Vector3 leftPosition = transform.position + Vector3.Cross(transform.forward, Vector3.up) * (laneWidth / 2f);
        leftPosition.y += wallHeight / 2f;
        leftWall.transform.position = leftPosition;
        leftWall.transform.rotation = Quaternion.LookRotation(transform.forward);
        ...
        Vector3 rightPosition = transform.position - Vector3.Cross(transform.forward, Vector3.up) * (laneWidth / 2f);
        rightPosition.y += wallHeight / 2f;
        rightWall.transform.position = rightPosition;
        rightWall.transform.rotation = Quaternion.LookRotation(transform.forward);
    ...
    private GameObject CreateCheckpoint(string name, Vector3 position, Color debugColor, string tag, string layer)
    {
        ...
        checkpoint.transform.rotation = Quaternion.LookRotation(transform.forward);
    ...
    {
        Vector3 arrowStart = transform.position - transform.forward * (laneLength * 0.5f) + Vector3.up * arrowYOffset;
        Vector3 arrowEnd   = transform.position + transform.forward * (laneLength * 0.5f) + Vector3.up * arrowYOffset;
